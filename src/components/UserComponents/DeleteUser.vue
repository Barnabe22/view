<template>
    <div>
        <h2 class="rapportTitleStyle">Suppression du collaborateur # {{userId}}</h2>
        <div class="contentStyle" v-if="userId != this.$store.state.user.id">
            <strong class="demandStyle">Voulez vous vraiment supprimer le collaborateur {{userId}} ? </strong> 
        ...
        <div class="contentStyle" v-else>
            <strong class="demandStyle">Vous ne pouvez pas vous supprimer vous même </strong> 
            <div class="inline-block btnStyle">
                <b-button variant="secondary" v-on:click="redirect" class="cancelBtnStyle">Retour</b-button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        performDelete(){
            ...
            Vue.axios.delete(this.$store.state.baseUrl+"/users/" + this.userId, {headers})
            .then(() => {
