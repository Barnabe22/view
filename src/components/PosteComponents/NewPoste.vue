<template>
  <div>
    ...
    <div class="textareaStyle">
      <label for="poste-state">Poste</label>
      <b-form-input id="poste-state" v-model="poste.name" placeholder="Entrez un poste"></b-form-input>
...
export default {
  name: "NewPoste",
  props: {
    pagetitle: {type: String,default: "Nouveau poste"},
  ...
  methods: {
    ...
    savePoste() {
      ...
      if (this.mode == "POST_MODE") {
        Vue.axios
          .post(this.$store.state.baseUrl + "/postes", this.poste, {headers})
          .then(() => {
              this.showMsgBox("Confirmation", "Poste enregistré avec succès", "success");
          }).catch(()=>{
              this.globalRefreshShowMsgBox('Info','C\'est possible qu\'il y ait une erreur\n Actualisation de la session','success')
        });
      } else if (this.mode == "UPDATE_MODE") {
        Vue.axios
          .patch(this.$store.state.baseUrl + "/postes/"+this.poste.id, this.poste,{headers})
          .then(() => {
    ...
    redirect() {
      this.$router.push({ name: "postesRoute" });
    },
  },
  beforeMount(){
